...
from app.infrastructure.repositories.producto_repo import get_producto_id_by_name
...
"""
    ...
    try:
        ...
        for entrada in precios:
            ...
            id_producto = await get_producto_id_by_name(estilo)
            if id_producto is None:
                faltantes.append(f"producto no encontrado: {estilo}")
                continue

            precios_limpios.append({
                "nombre_producto": estilo,
                "formato": formato,
                "valor": entrada.get("precio_litro"),
